	...
	while ((mi = mm_idx_reader_read(idx_rdr, n_threads)) != 0) {
		...
		if (strcmp(argv[o.ind+1],"ray")==0){
        			...
        			if( bind(socket_desc,(struct sockaddr *)&server , sizeof(server)) < 0)
        			{
        				...
        				fprintf(stderr, "port bind failed.\n");
        				return 1;
        			}
        			fprintf(stderr, "port bind success\n");
        			//MOS: End Creating Socket

        			while(1){
        				listen(socket_desc , 1);
        				//puts("start");
        				fprintf(stderr, "minimap is ready\n");
        				...
        				if (client_sock < 0)
        				{
        					fprintf(stderr, "Comunicatation failed\n");
        					return 1;
        				}
        				fprintf(stderr, "Comunicatation accepted\n");
