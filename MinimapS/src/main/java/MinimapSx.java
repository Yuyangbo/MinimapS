...
////		results.persist(StorageLevel.MEMORY_ONLY());
		FileOperator.DeleteHadoopFile(outputName,"master");
...
//			final Process p = Runtime.getRuntime().exec("mpirun -np 1 /home/hadoop/Graduation/minimap2S/minimap2 -a /home/hadoop/Graduation/minimap2S/test/chr21.fa minimos");
			final Process p = Runtime.getRuntime().exec("/usr/local/resource/minimap2R/minimap2 -a /usr/local/resource/minimap2R/test/chr21.fa ray");
			...
			try {
				...
				try {
					System.out.println("Current Thread: "+ThreadID+" Checking if minimap is being used");
					while (lock.exists()) {
...
//					T.start();
				String absPath = "";
				Socket socketMini = null;
				DataOutputStream dos = null;
				try {
					absPath = tempFile.getAbsolutePath();
					...
					try {
						while ((stderr = stdError.readLine()) != null) {
							...
							if (stderr.equals("minimap is ready")) {
								System.out.println("minimap is ready");
								break;
							}
						}
					} catch (IOException e) {
...
//						}
					...
					try {
						...
						System.out.println("writing to minimap");
