...
class minimapR2(object):
    def __init__(self, reads,options,tar,dest,corenum):
        self.reads=reads
        self.tar=tar
    ...
    def minimap(self):
        ...
        start = time.time()
        path="/usr/local/resource/tmp/Thread_"+str(os.getpid())+".fastq";
        ...
        p.wait()
        savepath = self.dest+"/part_" + str(self.corenum) + ".fastq";
